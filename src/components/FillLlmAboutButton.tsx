...
async function fillOnce(body: Record<string, unknown>): Promise<FillResult> {
  const res = await fetch("/api/llm-about-fill", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
  });
  const json = (await res.json()) as FillResult;
  if (!res.ok) throw new Error(json.message || "LLM about fill failed");
  return json;
}

export function FillLlmAboutButton({
  market,
  tickers,
  gapCount = 0,
  totalGaps = 0,
  ...
  const runPage = useCallback(async () => {
    setBusy(true);
    setProgress({ pct: 12, label: "LLM about", detail: `${gapCount} names` });
    try {
      const json = await fillOnce({
        market,
        tickers: tickers?.length ? tickers : undefined,
        limit: tickers?.length ? Math.min(tickers.length, 20) : 8,
      });
      await finish({
        pct: json.remaining <= 0 ? 100 : 90,
        label: json.remaining <= 0 ? "Complete" : "Partial",
        detail: json.message || `+${json.saved} about`,
  ...
  const runAll = useCallback(async () => {
    ...
    try {
      for (let round = 1; round <= 40; round += 1) {
        ...
        const json = await fillOnce({ market, limit: 12 });
        saved += json.saved;
      ...
      await finish({
        pct: 100,
        label: "Failed",
        detail: "LLM about batch stopped",
        error: true,
      });
    }
