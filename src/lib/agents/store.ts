...
export function lastQuantRun(): {
  ...
  const row = getDb()
    .prepare(
      `SELECT id, mode, engine, universe_count, debate_count, buy_count, finished_at
       FROM agent_runs
       WHERE engine LIKE 'quant%'
       ORDER BY id DESC
       LIMIT 1`,
    )
  ...
  return {
    ...
    debate_count: Number(row.debate_count),
    buy_count: Number(row.buy_count),
    finished_at: row.finished_at != null ? String(row.finished_at) : null,
  };
}

export function listVerdictsForRun(runId: number): StoredVerdict[] {
  const rows = getDb()
    .prepare(
      `SELECT * FROM agent_verdicts WHERE run_id = ? ORDER BY confidence DESC, id DESC`,
    )
