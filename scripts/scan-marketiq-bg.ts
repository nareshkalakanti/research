/**
 * Background MarketIQ scan — separate Node process (not next dev).
 * Resumes automatically: skips URLs already scored in marketiq.db.
 *
...
async function main() {
  ...
  npm run scan:marketiq-bg -- --days 7 --limit 40
  npm run scan:marketiq-bg -- --pending-only --limit 40
  npm run scan:marketiq-bg -- --days 7 --limit 40 --ocr

Re-run the same command after a crash — already-scored PDFs are skipped.`);
    return;
  }
  ...
  const skipOcr = !hasFlag("--ocr");

  const logDir = path.join(process.cwd(), "logs");
  fs.mkdirSync(logDir, { recursive: true });
  const jsonlPath = path.join(logDir, `marketiq-scan-${dayStamp()}.jsonl`);
  const append = (obj: Record<string, unknown>) => {
    fs.appendFileSync(jsonlPath, `${JSON.stringify(obj)}\n`, "utf8");
  };

  console.log(
    `marketiq-bg · days=${days} limit=${limit} batch=${batch} skipOcr=${skipOcr} pendingOnly=${pendingOnly}`,
  );
  console.log(`log → ${jsonlPath}`);

  let sources: MarketIqHit[] = [];
  if (!pendingOnly) {
    const found = await discoverMarketIqAnnounced(days);
    const saved = saveMarketIqHits(found.sources);
  ...
  // Always merge DB pending so resume picks up prior stubs
  const pending = listMarketIqPendingHits();
  const byUrl = new Map<string, MarketIqHit>();
  ...
  while (done < limit && sources.length) {
    ...
    const result = await scanMarketIqAnnouncements({
      days,
      limit: thisBatch,
      pendingOnly: true,
      sources,
    ...
    for (const r of result.results) {
      ...
      append({
        ...
        error: r.error ?? null,
        sentiment: r.extract?.sentiment ?? null,
      });
    }
